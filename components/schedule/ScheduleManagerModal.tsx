...
import {
    X, Trash2, CheckCircle2, Pen, Pencil, Check, AlertTriangle,
    Table, BookOpenCheck, AlertCircle
} from 'lucide-react';
...
const ScheduleManagerModal: React.FC<ScheduleManagerModalProps> = ({
    ...
    const handleConfirmAction = () => {
        ...
        if (confirmAction.mode === 'delete') {
            ...
            onUpdateSettings({
                ...
                activeScheduleId: isDeletingActiveSchedule ? undefined : activeScheduleId,
    ...
    const stats = [
        { label: 'إجمالي جداول الحصص', value: String(scheduleGenerationCount) },
        { label: 'إجمالي جداول الانتظار', value: String(waitingGenerationCount) },
        { label: 'الجداول المحفوظة', value: `${savedSchedules.length} / 10` },
        { label: 'الجدول المعتمد', value: activeSchedule?.name ?? '—', isText: true },
    ];
    ...
    return (
        ...
        >
            ...
            >
                <div className="flex items-center justify-between px-6 py-5 bg-slate-50 border-b border-slate-100 shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-bold text-slate-500 mt-0.5">عرض وإدارة جميع الجداول المُنشأة</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="px-6 pt-5 pb-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4 shrink-0 border-b border-slate-100">
                    {stats.map((s, i) => (
                        <div
                            key={i}
                        ...
                        >
                            ...
                            <p
                                className={`mt-3 font-black text-[#655ac1] text-center ${s.isText ? 'text-base' : 'text-[32px] leading-none'}`}
                                title={s.value}
                            >
                                {s.value}
                            </p>
