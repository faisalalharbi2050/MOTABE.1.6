...
const WorkingHoursCard: React.FC = () => (
  <div className="bg-gradient-to-l from-amber-50 to-white rounded-2xl border border-amber-200 p-5 flex items-start gap-4 shadow-sm">
    <div className="w-11 h-11 bg-amber-100 rounded-xl flex items-center justify-center shrink-0">
      <Clock size={22} className="text-amber-600" />
    </div>
    <div>
      <h4 className="font-black text-slate-800 text-base mb-0.5">أوقات العمل الرسمية</h4>
      <p className="text-sm font-bold text-slate-700">
        الأحد – الخميس &nbsp;|&nbsp; <span dir="ltr">8:00 ص — 2:30 م</span>
      </p>
      <p className="text-xs text-slate-500 font-medium mt-1">
        يُرجى رفع تذكرتك وسنتواصل معك خلال أوقات العمل. يمكن استخدام المساعد الذكي في أي وقت.
      </p>
    </div>
    <div className="shrink-0 flex flex-col items-center">
      <span className="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse" />
      <span className="text-[10px] font-bold text-green-600 mt-1 whitespace-nowrap">متاح الآن</span>
    </div>
  </div>
);
...
const TicketSection: React.FC<TicketSectionProps> = ({ openFormOnMount = false }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {!showForm && (
        <div className="flex justify-end">
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 bg-[#655ac1] text-white rounded-xl font-black hover:bg-[#52499d] hover:-translate-y-0.5 transition-all shadow-md shadow-indigo-200"
          >
            <PlusCircle size={18} />
            رفع تذكرة جديدة
          </button>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-[#655ac1]/20 shadow-md overflow-hidden animate-fade-in">
          <div className="px-6 py-4 bg-gradient-to-l from-[#f0eeff] to-white border-b border-slate-100 flex items-center justify-between">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 placeholder:text-slate-400 focus:outline-none focus:border-[#655ac1] focus:ring-1 focus:ring-[#655ac1]/30 transition-all"
              />
            </div>
              ...
              <div className="flex gap-3 items-start">
                <div className="relative flex-1">
                  <select
                    ...
                    className="w-full appearance-none px-4 py-3 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 focus:outline-none focus:border-[#655ac1] focus:ring-1 focus:ring-[#655ac1]/30 bg-white transition-all pr-10"
                  >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 placeholder:text-slate-400 focus:outline-none focus:border-[#655ac1] focus:ring-1 focus:ring-[#655ac1]/30 resize-none transition-all"
              />
            </div>
            ...
            <div>
              ...
              <div
                ...
                className={`border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-all
                  ${isDragging ? 'border-[#655ac1] bg-[#f0eeff]' : 'border-slate-200 hover:border-[#655ac1]/50 hover:bg-slate-50'}`}
              >
                <Upload size={24} className={`mx-auto mb-2 ${isDragging ? 'text-[#655ac1]' : 'text-slate-400'}`} />
            ...
            <div className="flex gap-3 pt-2">
              <button
                type="submit"
                className="flex-1 py-3 bg-[#655ac1] text-white rounded-xl font-black hover:bg-[#52499d] transition-all shadow-sm shadow-indigo-200 flex items-center justify-center gap-2 hover:-translate-y-0.5"
              >
                <Send size={16} />
                إرسال التذكرة
              </button>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
          ...
          <span className="bg-[#f0eeff] text-[#655ac1] text-xs font-black px-3 py-1 rounded-full">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/30">
                  <th className="text-right px-5 py-3 font-black text-slate-600 text-xs">رقم التذكرة</th>
                  <th className="text-right px-5 py-3 font-black text-slate-600 text-xs">الموضوع</th>
                  <th className="text-right px-5 py-3 font-black text-slate-600 text-xs">التصنيف</th>
                  <th className="text-right px-5 py-3 font-black text-slate-600 text-xs">الأولوية</th>
                  <th className="text-right px-5 py-3 font-black text-slate-600 text-xs">الحالة</th>
                  <th className="text-right px-5 py-3 font-black text-slate-600 text-xs">التاريخ</th>
                  <th className="text-right px-5 py-3 font-black text-slate-600 text-xs">المرفقات</th>
                </tr>
              </thead>
              <tbody>
                {tickets.map((ticket, i) => {
                  ...
                  return (
                    <tr key={ticket.id} className={`border-b border-slate-50 hover:bg-slate-50/50 transition-colors ${i % 2 === 0 ? '' : 'bg-slate-50/20'}`}>
                      {/* ID */}
                      <td className="px-5 py-4">
                        <span className="font-black text-[#655ac1] text-xs bg-[#f0eeff] px-2 py-1 rounded-lg">
                          {ticket.id}
                        </span>
                      </td>
                      {/* Title */}
                      ...
                      {/* Category */}
                      <td className="px-5 py-4">
                        <span className="text-xs font-bold text-slate-600">{ticket.categoryLabel}</span>
                      </td>
                      {/* Priority */}
                      <td className="px-5 py-4">
                        {ticket.priority === 'urgent' ? (
                          <span className="flex items-center gap-1 text-xs font-black text-red-600 bg-red-50 px-2 py-1 rounded-lg border border-red-200 w-fit">
                            <AlertCircle size={12} />
                            عاجل
                          </span>
                        ) : (
                          <span className="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded-lg">عادي</span>
                        )}
                      </td>
                      {/* Status */}
                      <td className="px-5 py-4">
                        <div>
                          <span className={`flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-lg border w-fit ${sc.color}`}>
                            <StatusIcon size={12} />
                            {sc.label}
                          </span>
                      ...
                      <td className="px-5 py-4">
                        ...
                        <p className="text-xs text-slate-400 font-medium">{ticket.time}</p>
                      </td>
                      {/* Attachments */}
                      <td className="px-5 py-4">
                            ...
                            {ticket.attachments.map((att, ai) => (
                              <div key={ai} title={att.name}
                                className={`w-7 h-7 rounded-lg flex items-center justify-center
                                  ${att.type === 'image' ? 'bg-blue-100 text-blue-600' : att.type === 'pdf' ? 'bg-red-100 text-red-600' : 'bg-indigo-100 text-indigo-600'}`}>
