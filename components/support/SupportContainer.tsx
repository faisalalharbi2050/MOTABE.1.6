...
import { Headphones, TicketIcon, BookOpen, MessageCircle, Bell } from 'lucide-react';
        ...
        <div className="relative z-10 flex items-start justify-between gap-4 flex-col sm:flex-row">
          ...
          <div className="relative shrink-0" ref={notifRef}>
            ...
            >
              <Bell size={20} />
              {/* Unread badge */}
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#655ac1] text-white text-[10px] font-black rounded-full flex items-center justify-center">
                2
              </span>
            </button>
            {showNotif && (
      ...
      <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200 flex gap-2 overflow-x-auto custom-scrollbar">
        {TABS.map(tab => (
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold whitespace-nowrap transition-all flex-1 justify-center text-sm
              ${activeTab === tab.id
