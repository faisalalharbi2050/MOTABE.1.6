...
const ClassSubjectOverridesModal: React.FC<Props> = ({ isOpen, onClose, classes, setClasses, subjects, setSubjects, gradeSubjectMap, assignments, setAssignments, scheduleSettings, setScheduleSettings, schoolInfo, activePhase, activeSchoolId, initialClassIds = [], onSaved }) => {
  ...
  const singleClassMode = initialClassIds.length === 1;
  ...
  return <div className="fixed inset-0 z-[10000] bg-slate-900/55 backdrop-blur-sm flex items-center justify-center p-3" dir="rtl">
    <div className={`bg-slate-50 w-full ${singleClassMode ? 'max-w-4xl' : 'max-w-6xl'} h-[92vh] rounded-[2rem] shadow-2xl overflow-hidden flex flex-col border border-slate-200`}>
      <header className="bg-white px-6 py-4 border-b border-slate-100 flex items-center justify-between"><div className="flex items-center gap-3"><ListPlus size={27} className="text-[#655ac1]" /><div><h2 className="text-lg font-black text-slate-800">{singleClassMode ? 'تخصيص مواد الفصل' : 'تخصيص مواد الفصول'}</h2><p className="text-[11px] font-bold text-slate-400">{singleClassMode && selectedClass ? `فصل ${classNameOf(selectedClass)}` : 'خصّص وعدّل مواد الفصول.'}</p></div></div><button onClick={onClose} className="w-9 h-9 rounded-full border border-slate-300 text-slate-500 flex items-center justify-center"><X size={18} /></button></header>
      <div className="flex-1 flex overflow-hidden">
        {!singleClassMode && <aside className="w-72 shrink-0 bg-white border-l border-slate-100 flex flex-col"><div className="flex-1 overflow-y-auto p-2 space-y-1">{eligibleClasses.map(c => { const selected = c.id === selectedId; const custom = !!c.subjectIdsCustomized || Object.keys(c.subjectPeriodOverrides || {}).length > 0; return <button key={c.id} onClick={() => loadClass(c.id)} className={`w-full text-right px-3 py-2.5 rounded-xl border flex items-center justify-between ${selected ? 'border-slate-300 shadow-sm' : 'border-transparent hover:bg-slate-50'}`}><span><b className={`block text-sm ${selected ? 'text-[#655ac1]' : 'text-slate-700'}`}>{classNameOf(c)}</b><small className="text-[10px] font-bold text-slate-400">الصف {c.grade}{custom ? ' · مخصص' : ''}</small></span><RoundCheck selected={selected} /></button>; })}</div></aside>}
        <main className="flex-1 overflow-y-auto p-5">{!selectedClass ? <div className="h-full grid place-items-center text-sm font-black text-slate-400">اختر فصلًا من القائمة</div> : <div className="max-w-3xl mx-auto space-y-4">
          <section className="bg-white rounded-2xl border border-slate-300 shadow-sm p-4"><b className="block text-sm text-slate-800">الفصل المستهدف</b><p className="text-sm font-black text-[#655ac1] mt-1">{classNameOf(selectedClass)}</p></section>
          ...
          <section className="space-y-2">
            ...
            {activeSubjects.map((s, index) => {
              ...
              return <div key={s.id} className="bg-white rounded-2xl border border-slate-200 p-3 grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_160px_138px_132px] items-center gap-3">
                <div className="flex min-w-0 items-center gap-2"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-50 text-xs font-bold text-slate-400">{index + 1}</span><b className="min-w-0 text-sm text-slate-800 truncate">{s.name}</b></div>
                ...
                <div className="grid grid-cols-[62px_64px] items-center gap-2"><label className="whitespace-nowrap text-[10px] font-black text-slate-400">نصاب المادة</label><input type="number" min={1} max={99} value={value} onChange={e => setPeriods(s, Number(e.target.value))} className="h-9 w-16 rounded-xl border-2 border-slate-200 px-2 text-center text-sm font-black text-[#655ac1] outline-none focus:border-[#655ac1]/40" /></div>
            ...
            <div className="grid grid-cols-1 items-center gap-3 rounded-2xl border border-slate-200 bg-white p-3 md:grid-cols-[minmax(0,1fr)_160px_138px_132px]">
              <div className="min-w-0"><b className="text-sm text-slate-700">إجمالي المواد: <span className="text-[#655ac1]">{activeSubjects.length}</span></b></div>
              <div className="hidden md:block" />
              <div className="grid grid-cols-[62px_64px] items-center gap-2"><label className="whitespace-nowrap text-[10px] font-black text-slate-400">إجمالي الحصص</label><span className="w-16 text-center text-lg font-black text-[#655ac1]">{total}</span></div>
              <div className="hidden md:block" />
            </div>
          </section>
