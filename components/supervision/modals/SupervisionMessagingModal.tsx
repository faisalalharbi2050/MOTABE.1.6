...
function buildSignLink(token: string): string {
  ...
  return `${base}?supervisionSign=${encodeURIComponent(token)}`;
}

function buildElectronicMsgSupervisor(
  staffType: 'teacher' | 'admin', day: string,
  locationNames: string[], effectiveDate: string | undefined, link: string
): string {
  const title = staffType === 'teacher' ? 'المعلم الفاضل' : 'الإداري الفاضل';
  const locs = locationNames.length > 0 ? locationNames.join(' و') : '—';
  const date = effectiveDate
    ? new Intl.DateTimeFormat('ar-SA', { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(effectiveDate))
    : '';
  const datePart = date ? ` بدءاً من ${date}` : '';
  return `${title}، نشعركم بإسناد مهمة الإشراف اليومي لكم في يوم ${DAY_NAMES[day] || day} في المواقع: ${locs}${datePart}.\nالرجاء التوقيع عبر الرابط:\n${link}`;
}

function buildElectronicMsgFollowUp(
  day: string, effectiveDate: string | undefined, link: string
): string {
  const date = effectiveDate
    ? new Intl.DateTimeFormat('ar-SA', { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(effectiveDate))
    : '';
...
interface PreviewProps {
  ...
  schoolInfo: SchoolInfo;
  onClose: () => void;
  onSigned: (key: string, rowType: 'supervisor' | 'followup', day: string, staffId: string, sig: string) => void;
}
const SignaturePreviewModal: React.FC<PreviewProps> = ({ row, supervisionData, schoolInfo, onClose, onSigned }) => {
  ...
  const handleConfirm = () => {
    ...
    setConfirmed(true);
    setTimeout(() => { onSigned(row.key, row.rowType, row.day, row.staffId, data); onClose(); }, 1800);
  };

  const locs = row.locationNames.join(' و') || '—';
  const dayName = DAY_NAMES[row.day] || row.day;
  const effDate = supervisionData.effectiveDate
    ? new Intl.DateTimeFormat('ar-SA', { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(supervisionData.effectiveDate))
    : '';
        ...
        <div className="p-5 bg-slate-50 overflow-y-auto space-y-4">
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
            ...
            <div className="space-y-2.5 text-sm">
              ...
              {row.rowType === 'supervisor' && (
                <div className="flex justify-between items-center py-1.5 border-b border-slate-50">
                  <span className="text-slate-500 font-medium">موقع الإشراف:</span>
                  <span className="font-black text-slate-800 text-right max-w-[55%]">{locs}</span>
                </div>
              )}
              {row.rowType === 'followup' && (
                <div className="flex justify-between items-center py-1.5 border-b border-slate-50">
                  <span className="text-slate-500 font-medium">المهمة:</span>
                  <span className="font-black text-slate-800">مشرف متابع</span>
                </div>
              )}
              {effDate && (
...
const SupervisionMessagingModal: React.FC<Props> = ({
  ...
  const electronicRows = useMemo((): ElectronicRow[] => {
    ...
    days.forEach(day => {
      ...
      da.staffAssignments.forEach(sa => {
        const locs = sa.locationIds.map(lid => supervisionData.locations.find(l=>l.id===lid)?.name||'').filter(Boolean);
        const tok = sa.signatureToken || buildToken(sa.staffId, day);
        const key = `elec-sup-${day}-${sa.staffId}`;
        const base = buildElectronicMsgSupervisor(sa.staffType, day, locs, supervisionData.effectiveDate, buildSignLink(tok));
        result.push({ key, rowType:'supervisor', staffId:sa.staffId, staffName:sa.staffName, staffType:sa.staffType, day, locationNames:locs, message:customMessages[key]??base, token:tok, signatureData:sa.signatureData, signatureStatus:sa.signatureStatus });
      });
      if (da.followUpSupervisorId && da.followUpSupervisorName) {
        const tok = da.followUpSignatureToken || buildToken(da.followUpSupervisorId+'_fu', day);
        const key = `elec-fu-${day}-${da.followUpSupervisorId}`;
        const base = buildElectronicMsgFollowUp(day, supervisionData.effectiveDate, buildSignLink(tok));
        result.push({ key, rowType:'followup', staffId:da.followUpSupervisorId, staffName:da.followUpSupervisorName, staffType:'admin', day, locationNames:[], message:customMessages[key]??base, token:tok, signatureData:da.followUpSignatureData, signatureStatus:da.followUpSignatureStatus });
      }
    });
    return result;
  ...
  const textRows = useMemo(() => {
    ...
    days.forEach(day => {
      ...
      da.staffAssignments.forEach(sa => {
        const locs = sa.locationIds.map(lid=>supervisionData.locations.find(l=>l.id===lid)?.name||'').filter(Boolean);
        const key = `text-${day}-${sa.staffId}`;
        const base = generateAssignmentMessage(sa.staffName, sa.staffType, day, locs, supervisionData.effectiveDate, schoolInfo.gender??'بنين');
        result.push({ key, staffId:sa.staffId, staffName:sa.staffName, staffType:sa.staffType, day, locationNames:locs, message:customMessages[key]??base });
      });
    });
    return result;
  ...
  const reminderRows = useMemo(() => {
    ...
    days.forEach(day => {
      ...
      da.staffAssignments.forEach(sa => {
        const locs = sa.locationIds.map(lid=>supervisionData.locations.find(l=>l.id===lid)?.name||'').filter(Boolean);
        const key = `rem-${day}-${sa.staffId}`;
        const base = generateReminderMessage(sa.staffName, sa.staffType, day, locs, schoolInfo.gender??'بنين');
        result.push({ key, staffId:sa.staffId, staffName:sa.staffName, staffType:sa.staffType, day, locationNames:locs, message:customMessages[key]??base });
      });
    });
    return result;
  ...
  const markPending = (row: ElectronicRow) => {
    setSupervisionData(prev => ({
      ...
      dayAssignments: prev.dayAssignments.map(da => {
        if (da.day !== row.day) return da;
        if (row.rowType === 'supervisor') {
          return { ...da, staffAssignments: da.staffAssignments.map(sa => sa.staffId===row.staffId ? {...sa,signatureStatus:'pending' as const,signatureToken:row.token} : sa) };
        } else {
          return { ...da, followUpSignatureStatus:'pending' as const, followUpSignatureToken:row.token };
        }
      })
    }));
  };

  const handleSigned = (key:string, rowType:'supervisor'|'followup', day:string, staffId:string, sig:string) => {
    setSupervisionData(prev => ({
      ...prev,
      dayAssignments: prev.dayAssignments.map(da => {
        if (da.day!==day) return da;
        if (rowType==='supervisor') {
          return { ...da, staffAssignments: da.staffAssignments.map(sa => sa.staffId===staffId ? {...sa,signatureData:sig,signatureStatus:'signed' as const} : sa) };
        } else {
          return { ...da, followUpSignatureData:sig, followUpSignatureStatus:'signed' as const };
        }
      })
    }));
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 space-y-4">
              ...
              <div className="flex gap-3 items-start">
                <div className="flex-1">
                  ...
                  <textarea value={masterTemplate} onChange={e=>setMasterTemplate(e.target.value)} placeholder="اكتب نصاً واضغط (اعتماد للكل)..." rows={3}
                ...
                <button onClick={applyMaster} disabled={!masterTemplate.trim()}
                  className="mt-6 flex items-center gap-2 bg-[#655ac1] hover:bg-[#4e44a6] text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-sm active:scale-95 disabled:opacity-40 shrink-0">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              ...
              <div className="px-5 py-3 border-b border-[#655ac1]/10 bg-[#f3f0ff]/40 flex items-center gap-3">
                ...
                <div className="relative">
                  <select value={selectedDay} onChange={e=>{setSelectedDay(e.target.value);setSelectedIds(new Set());}}
                    className="appearance-none pl-9 pr-4 py-2 rounded-xl border-2 border-[#655ac1]/30 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-[#655ac1]/30 focus:border-[#655ac1] outline-none bg-white cursor-pointer shadow-sm hover:border-[#655ac1] transition-colors min-w-[160px]">
                    <option value="all">📅 كل الأيام</option>
                    {activeDays.map(d=><option key={d} value={d}>{DAY_NAMES[d]}</option>)}
                  </select>
                  ...
                  <table className="w-full text-sm text-right">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50">
                        ...
                        <th className="px-3 py-3 font-black text-slate-700 text-xs text-center w-24">الصفة</th>
                        ...
                        return (
                          <tr key={row.key} className={`hover:bg-slate-50/60 transition-colors ${checked?'bg-[#f3f0ff]/60':''}`}>
                            ...
                            <td className="px-3 py-3">
                              <span className="font-bold text-slate-800 text-sm">{row.staffName}</span>
                              {eRow?.rowType==='followup' && <span className="block text-[10px] text-amber-600 font-bold mt-0.5">مشرف متابع</span>}
                            </td>
                            <td className="px-3 py-3 text-center">
                              <span className={`text-sm font-bold ${row.staffType==='teacher'?'text-blue-700':'text-[#655ac1]'}`}>
                                {row.staffType==='teacher'?'معلم':'إداري'}
                              </span>
                            </td>
                            <td className="px-3 py-3 text-center">
                              <span className="text-sm font-bold text-[#655ac1]">{DAY_NAMES[row.day]}</span>
                            </td>
